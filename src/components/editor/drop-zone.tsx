...
export function DropZone({ onFile, compact }: Props) {
  ...
  return (
    <motion.div
      ...
      className="w-full max-w-[200px] mx-auto py-3 px-4 flex flex-col items-center justify-center gap-1.5 cursor-pointer bg-[rgba(255,255,255,0.015)] border-[1.5px] border-dashed border-[rgba(255,255,255,0.08)] transition-all duration-300 hover:border-[var(--accent)] hover:bg-[var(--accent)]/5 group"
      onDragOver={(e) => e.preventDefault()}
      onDrop={handleDrop}
    >
      <label htmlFor={UPLOAD_ID} className="flex flex-col items-center justify-center gap-2 cursor-pointer w-full h-full">
        <div className="w-6 h-6 flex items-center justify-center bg-[rgba(255,255,255,0.03)] border border-[rgba(255,255,255,0.06)] group-hover:border-[var(--accent)]/30 group-hover:bg-[var(--accent)]/8 transition-all duration-300">
        ...
        <motion.span
          ...
          whileTap={{ scale: 0.97 }}
          className="bg-[var(--accent)]/15 text-[var(--accent)] border border-[var(--accent)]/20 px-3 py-1.5 font-extrabold text-xs cursor-pointer transition-all duration-300 hover:bg-[var(--accent)]/20 shadow-none"
        >
          Upload Your Image
        </motion.span>
